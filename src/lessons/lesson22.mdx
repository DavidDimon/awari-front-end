...
<Slide title="Introdução(GraphQL)">
  ...
  backend, por exemplo: As vezes uma tela no web(desktop) carregam mais dados
  diferentes tais como: os usuários que deram like em um post, informação que,
  no mobile só aparece se clicarmos, e formos para uma outra tela.
</Slide>
...
<Slide title="Aplicação com GraphQL">
  <a
    href="https://codesandbox.io/s/graphql-example-ew5sq6?file=/src/App.js"
    target="_blank"
  >
...
<Slide title="Regras gerais - Siga as convenções">
  ...
  todas as constantes, com letras maiusculas, continue nesse padrão, acontece
  muito de criarmos um padrão no inicio do projeto, porém ao decorrer do
  desenvolvimento abandonarmos esse padrão, para facilitar a codificação.
</Slide>
